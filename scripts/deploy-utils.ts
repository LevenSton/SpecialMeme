...
export const getImplAddr = async (hre: HardhatRuntimeEnvironment) =>  {
  const chainId = hre.network.config.chainId;
  if(chainId === 59140){
    return process.env.ADDRESS_LINEA_TESTNET  || ''
  }else if(chainId === 59144){
    return process.env.ADDRESS_LINEA_MAINNET || ''
  }
  return ''
}
...
const SIGN_PRIVATEKEY = "0x0b97666250ff66b3e6517a871600ad1ca82317c897dbec4bf09ee797836aff56"
...
export async function buildTransferSeparator(
  ...
  const msgParams = buildTransferKeyParams(coinAddress, name, subject, from, to, amount);
...
export async function buildBuyStandardKeySeparator(
  ...
  const msgParams = buildBuyStandardKeyParams(coinAddress, name, subject, userAddress, amount);
  return await getSig(msgParams);
}
...
export async function buildBuySeparator(
  ...
  const msgParams = buildBuyKeyParams(coinAddress, name, subject, userAddress, amount);
  return await getSig(msgParams);
}
...
export async function buildMintSeparator(
  ...
  const msgParams = buildMintParams(coinAddress, name, subjectAddress, userAddress, amount);
  return await getSig(msgParams);
}
