...
const deployFn: DeployFunction = async (hre) => {
  ...
  const proxyMemeCoinFactoryNonce = hexlify(toBeHex((deployerNonce + 2).toString()));
  const proxyAddress = '0x' + keccak256(encode([deployer, proxyMemeCoinFactoryNonce])).substring(26);
  ...
  await deployAndVerifyAndThen({
    hre,
    name: "MemeCoinManager",
    contract: 'MemeCoinManager',
    args: [swapRouter, proxyAddress],
  })
}

// This is kept during an upgrade. So no upgrade tag.
deployFn.tags = ['DeployMemeManager']
